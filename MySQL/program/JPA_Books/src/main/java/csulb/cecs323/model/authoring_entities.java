...
@NamedNativeQueries( value = {
        @NamedNativeQuery(name = "selectAllAuthoringEntities",
                          query = "SELECT * " +
                                  "FROM AUTHORING_ENTITIES ",
                          resultClass = authoring_entities.class),
        @NamedNativeQuery(name = "selectAuthoringEntityByEmail",
                          query = "SELECT * " +
                                  "FROM AUTHORING_ENTITIES " +
                                  "WHERE AUTHORING_ENTITY_TYPE = '" + "?" + "' ",
                          resultClass = authoring_entities.class),
        @NamedNativeQuery(name = "selectAllAdHocTeams",
                query = "SELECT * " +
                        "FROM AUTHORING_ENTITIES " +
                        "WHERE AUTHORING_ENTITY_TYPE = 'Ad Hoc Team' ",
                resultClass = authoring_entities.class),
        @NamedNativeQuery(name = "selectAllWritingGroups",
                          query = "SELECT * " +
                                  "FROM AUTHORING_ENTITIES " +
                                  "WHERE AUTHORING_ENTITY_TYPE = 'Writing Group' ",
                          resultClass = authoring_entities.class),
        @NamedNativeQuery(name = "selectAllIndividualAuthors",
                         query = "SELECT * " +
                                 "FROM AUTHORING_ENTITIES " +
                                 "WHERE AUTHORING_ENTITY_TYPE = 'Individual Author' ",
                        resultClass = authoring_entities.class)
    }// End of specific instances of NamedNativeQueries
)// End of all NamedNativeQueries
